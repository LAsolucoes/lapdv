  ...
  if (inputCheckboxAtivo.checked === true) {
    ...
    componetens.forEach(function (elementosHtml) {
      ...
      if (
        ...
        feedbackInativo.querySelector("p").innerHTML =
          "* Cliente Inativo ! * <br> Cadastro disponivel somente para leitura. Se necessário realizar alguma alteração, Habilite a opção 'ATIVO'. ";
      }
    });
  }
}

function bloqueiarCredito(){
 let checkboxBloqueiarCredito = document.getElementById('bloqueiar-credito')
 
 if (checkboxBloqueiarCredito.checked === true) {
  document.getElementById("limite-de-credito").disabled = true
  document.getElementById("credito-utilizado").disabled = true
  document.getElementById("limite-disponivel").disabled = true

} else if (checkboxBloqueiarCredito.checked === false) {
  document.getElementById("limite-de-credito").disabled = false
  document.getElementById("credito-utilizado").disabled = false
  document.getElementById("limite-disponivel").disabled = false
}
}
